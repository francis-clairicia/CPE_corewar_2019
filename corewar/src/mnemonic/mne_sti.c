...
static int get_index(args_type_t type, int value, int *reg)
{
    int index = 0;
...
int mne_sti(param_t const *params, champ_t *champ, battle_t *battle)
{
    ...
    first = get_index(params->type[1], params->value[1], champ->reg);
    second = get_index(params->type[2], params->value[2], champ->reg);
